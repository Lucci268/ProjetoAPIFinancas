...
import { 
  ...
  Check, Trash2, X, LogOut, Camera, PiggyBank, UserPlus, Edit2, AlertCircle, CheckCircle, HelpCircle, FileText, Calendar, Bell, Clock
} from 'lucide-react';
        ...
        <div className="col-span-1 animate-in slide-in-from-bottom-8 duration-500 delay-400">
           ...
           <div className="flex flex-col gap-3">
              {sortedReminders.length === 0 ? (
                ...
                sortedReminders.slice(0, 3).map((rem, idx) => {
              ...
              {sortedReminders.length > 0 && (
                  <div className="text-center mt-2">
                      <span className="text-[10px] text-gray-500">Gerencie todas na aba Planejamento</span>
                  </div>
              )}
           </div>
        </div>
...
const PlanningScreen = ({ goals, reminders, onAddGoal, onDeleteGoal, onDepositToGoal, onEditGoal, onAddReminder, onDeleteReminder, showAlert }) => {
...
function App() {
  ...
  const fetchUserData = async () => {
      ...
      try {
          ...
          const resReminders = await axios.get(`http://localhost:8080/api/reminders?userId=${user.id}`);
  ...
  const handleAddReminder = async (r) => {
    try {
        const res = await axios.post('http://localhost:8080/api/reminders', {
            userId: user.id,
        ...
        showAlert('Erro', 'Falha ao criar lembrete.', 'error');
    }
  };
      ...
      <div key={activeTab} className="animate-in fade-in slide-in-from-bottom-8 duration-500 ease-out">
        {(() => {
          switch(activeTab) {
             ...
             case 'createGoal': 
             case 'planejamento': return <PlanningScreen goals={goals} reminders={reminders} onAddGoal={handleAddGoal} onDeleteGoal={handleDeleteGoal} onDepositToGoal={handleDepositToGoal} onEditGoal={handleEditGoal} onAddReminder={handleAddReminder} onDeleteReminder={handleDeleteReminder} showAlert={showAlert} />;
